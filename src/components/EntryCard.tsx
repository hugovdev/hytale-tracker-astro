          ...
          <div className="bg-slate-100 dark:bg-slate-800 md:py-10 py-5 px-8 rounded-b-2xl w-full relative shadow-md">
               <div>
                    <div className="h-[290px] md:h-[230px]">
                         <div className="font-bold text-lg md:text-xl text-slate-900 dark:text-slate-200">{entry.title}</div>
                         <div className="pb-5 text-slate-500 dark:text-slate-400 text-sm md:text-base font-semibold">{"Published on " + new Date(entry.date).toLocaleDateString("en-US")} </div>
                         <div className="pb-5 text-slate-800 dark:text-slate-300 text-sm md:text-base font-medium" dangerouslySetInnerHTML={{ __html: entry.body }} />
                    </div>
